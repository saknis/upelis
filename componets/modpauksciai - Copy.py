...
#req.request.get('test')
    ...
    if req.request.get('cmd') == '1':
        ...
        for greeting in greetings:
          ...
          aaa=aaa+(u"<tr><td>Foto arialas</td><td><a href=\"https://www.google.lt/search?num=10&hl=lt&site=imghp&tbm=isch&source=hp&biw=1054&bih=330&q=%s\">Foto</a></td></tr>\n" % (greeting.rusislot+' distribution map'))
...
#          aaa=aaa+(u"<tr><td>Raudonojoje knygoje</td><td>%s</tr>\n" % (raud))
        aaa = aaa + "</tbody></table>\n"
        aaa = (u'<h1>Lietuvos pauk\u0161\u010Diai: %s</h1><p>' % (greeting.rusis))+aaa+"</p>"
        ...
        aaa = aaa + "</tbody></table>\n"
        aaa = u'<h1>Lietuvos pauk\u0161\u010Diai</h1><p>'+aaa+"</p>"
    result['cont']=aaa
    return aaa
  def name(self):
    return "lietuvospauksciai"
  def fpput(self):
    return True
  def title(self,req):
    if req.request.get('cmd') == '1':
        greetings = db.GqlQuery("SELECT * FROM Pauksciai  WHERE rusis = :1", req.request.get('paukstis'))
        for greeting in greetings:
          return u'Lietuvos pauk\u0161\u010Diai - ' + greeting.rusis
    return u'Lietuvos pauk\u0161\u010Diai'
  def descr(self):
    return u'   '
